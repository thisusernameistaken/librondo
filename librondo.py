...
def hook_addr(event):
    if event.data.target_stopped_data.reason == DebugStopReason.Breakpoint:
        hooks = context.active_dbg.hooks
        current_ip = context.active_dbg.dc.ip
        if current_ip in hooks.keys():
            hook = hooks[current_ip]
            # if hook.enabled:
                # hook.enabled=False
            hook.hit_count+=1
            hook.callback(context.active_dbg)
                # hook.enabled=True
...
class BinjaDBG:
    ...
    def __init__(self,path):
        self.bv = load(path,options={"analysis.mode":"controlFlow"})
